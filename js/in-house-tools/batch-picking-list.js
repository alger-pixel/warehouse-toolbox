(function(window) {
  ...
  const columns = [['pickingListNumber','Picking List'],['createdAt','Created'],['warehouse','Warehouse'],['clientId','Client'],['total','Packages'],['processed','Confirmed Complete'],['remaining','Not Confirmed Complete'],['commandCount','Commands'],['hasParts','Has Parts Used'],['status','Status'],['processTime','PROCESS TIME']];
  const esc = v => String(v ?? '').replace(/[&<>"']/g,c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const torontoDateTime=new Intl.DateTimeFormat('en-CA',{timeZone:'America/Toronto',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hourCycle:'h23'});
  function formatTorontoDateTime(value) {
    if(value===null||value===undefined||!String(value).trim())return '—';
    const date=new Date(value);
    if(Number.isNaN(date.getTime()))return esc(value);
    const parts=Object.fromEntries(torontoDateTime.formatToParts(date).map(part=>[part.type,part.value]));
    return `${parts.year}-${parts.month}-${parts.day} ${parts.hour}:${parts.minute}`;
  }
  function processTimeHtml(value, detail=false) {
    const raw=String(value ?? '');
    if(!raw.trim())return '—';
    const match=/^(COMPLETED|CANCELLED) \| ([1-9]\d*) MIN$/.exec(raw);
    if(!match)return esc(raw);
    return `${match[2]} min${match[1]==='CANCELLED'?(detail?' (Cancelled)':'<br>Cancelled'):''}`;
  }
  ...
  function workbook(data) {
    ...
    const lists=data.results.map(r=>Object.fromEntries(columns.map(([k,label])=>[label,k==='processTime'?(r[k]??''):r[k]])));
