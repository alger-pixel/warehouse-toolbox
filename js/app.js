(function (window, document) {
  ...
  function renderDashboard() {
    ...
    const dashboardClientCard = clientId => {
      ...
      const image = clientId === 'B044' ? 'assets/images/B044 Li4Pro battery image.png' : clientId === 'TINECO-TOC' ? 'assets/images/Tineco product image.png' : '';
      return `<button class="dashboard-client-card" type="button" data-route="client-tools" data-client-group="${escapeHtml(clientId)}"><span class="dashboard-client-icon">${clientId === 'B044' ? icons.parts : icons.repair}</span><span class="dashboard-client-copy"><span class="dashboard-client-title"><strong>${escapeHtml(clientId)}</strong><span class="dashboard-client-status">${active ? 'Active' : 'Unavailable'}</span></span><span>${group.length} ${group.length === 1 ? 'Client Tool' : 'Client Tools'}</span></span>${image ? `<img class="dashboard-client-product" src="${escapeHtml(image)}" alt="" loading="lazy" decoding="async">` : ''}</button>`;
    };
    mainContent.innerHTML = `<div class="dashboard-view">
      <section class="dashboard-intro" aria-labelledby="dashboard-title">
        <div class="dashboard-hero-copy"><span class="dashboard-eyebrow">Smart tools. Stronger operations.</span><h2 class="dashboard-title" id="dashboard-title"><img class="dashboard-brand-logo" src="assets/images/MKS LOGO.png" alt="MKS"><span>Warehouse Tools</span></h2><p class="dashboard-subtitle">Streamline today. A more efficient tomorrow.</p><ul class="dashboard-benefits">${benefitItems.map(([icon, label]) => `<li>${icon}<span>${label}</span></li>`).join('')}</ul></div>
