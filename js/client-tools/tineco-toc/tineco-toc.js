(function(window) {
  ...
  function statusPanel(){
    ...
    const saving=session?.pending?.action==='finish';
    if(saving||saveState==='failed'){
      ...
      summary=[['SN',session.sn],['UNIT ID',session.unitId],...(saving?[['Outcome',session.pending.body.outcome]]:[])];
    }else if(starting){
  ...
  async function run(action,body,recovering=false){
    if(busy)return;saveState=action==='finish'?(recovering?'recovering':session.pending?.action==='finish'?'retrying':'saving'):'';busy=true;error='';const gen=generation;
    ...
    try{const response=await window.MkiteApiClient.post(`/api/tineco-toc/${action}`,body);if(!ctx||generation!==gen)return;
      ...
      saveState='';
      if(session?.phase==='saved')ctx.audio?.success?.();
    }catch(e){if(ctx&&generation===gen){if(action==='finish')saveState='failed';error=e.message;ctx.toast.show(error);if(action==='begin'&&['SN_REQUIRED','SN_INVALID','DUPLICATE_SN_RECORDS','TINECO_TOC_RECORD_INVALID','TINECO_TOC_SCHEMA_INVALID','TINECO_TOC_TABLE_NOT_CONFIGURED','UNIT_ALREADY_ACTIVE'].includes(e.code)){session.startRejected=true;persist();}
      // Validation errors are known to precede writes. Unknown save outcomes stay locked.
      if(['ISSUE_FOUND_REQUIRED','INVALID_FORM','PAYLOAD_TOO_LARGE','SN_REQUIRED','SN_INVALID','INVALID_PART_QUANTITY','PART_DETAIL_REQUIRED','REPAIR_LEVEL_REQUIRED','FINAL_QC_FAILURE_REASON_REQUIRED','DISPOSAL_NOTE_REQUIRED','REPAIR_RESULT_OPTION_NOT_AVAILABLE','FINAL_QC_RESULT_OPTION_NOT_AVAILABLE','REPAIR_LEVEL_OPTION_NOT_AVAILABLE','TINECO_TOC_SCHEMA_INVALID','TINECO_TOC_TABLE_NOT_CONFIGURED'].includes(e.code)){if(action!=='begin'){delete session.pending;persist();}}
