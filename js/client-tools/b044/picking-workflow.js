(function (window, document) {
  ...
  function possiblePartsSummary(pl, inventory = {}) {
    ...
    const rows=parts.flatMap(part=>{
      const result=inventory[part.sku];
      if(result?.state==='ready'&&result.locations?.length)return result.locations.map((location,index)=>`<tr><td>${index?'':esc(part.sku)}</td><td>${index?'':part.quantity}</td><td>${esc(location.warehouseCode)}</td><td>${esc(location.locationCode)}</td><td>${location.availableQuantity===null?'—':esc(location.availableQuantity)}</td></tr>`);
      const status=result?.state==='ready'?['—','NOT FOUND','—']:result?.state==='error'?['—','LOCATION UNAVAILABLE','—']:['—','LOCATION PENDING','—'];
      return [`<tr><td>${esc(part.sku)}</td><td>${part.quantity}</td><td>${status[0]}</td><td>${status[1]}</td><td>${status[2]}</td></tr>`];
    });
    return `<section class="possible-parts-summary"><h2>POSSIBLE PARTS NEEDED SUMMARY</h2><table><thead><tr><th>PART / MATERIAL</th><th>POSSIBLE QTY</th><th>WAREHOUSE</th><th>LOCATION</th><th>AVAILABLE</th></tr></thead><tbody>${rows.length ? rows.join('') : '<tr><td colspan="5">No recognizable parts/materials found in commands.</td></tr>'}</tbody></table><p>Preparation estimate only. Confirm actual parts during Part Used Scan.</p></section>`;
  }
  ...
  function a4Markup(pl, inventory) {
    return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(pl.pickingListNumber)}</title><style>
      ...
      .possible-parts-summary{margin:3mm 0 4mm;break-inside:auto}.possible-parts-summary h2{font-size:10pt;margin:0 0 2mm;break-after:avoid}.possible-parts-summary th:first-child{width:30%}.possible-parts-summary th:nth-child(2){width:12%}.possible-parts-summary th:nth-child(3){width:15%}.possible-parts-summary th:nth-child(4){width:29%}.possible-parts-summary th:nth-child(5){width:14%}.possible-parts-summary th,.possible-parts-summary td{padding:1mm 1.2mm;font-size:7.5pt}.possible-parts-summary p{font-size:8pt;margin:1.5mm 0 0;break-before:avoid}
