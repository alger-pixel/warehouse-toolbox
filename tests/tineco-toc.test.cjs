...
for(const step of [0,1,2])test(`step ${step+1} shows PAUSE and requires a note before saving`,async()=>{
 const h=setup({phase:'draft',requestId:'visit-1234567890123456',recordId:'existing',unitId:'TCU-old',sn:'SN',entry:2,step,clientStatus:'Pending',previousLaborMinutes:15,startedAt:Date.now(),resumed:true,generalNote:'Earlier note',data:{trackingNumber:'TRACK',issueFound:'Issue',partUsedDetail:'Motor\nFilter',totalPartsUsed:'2',repairLevel:'Level 1',finalQcNote:'draft'}});
 assert.match(h.node('#tcu-app').innerHTML,/id="tcu-pause"/);assert.match(h.node('#tcu-app').innerHTML,/GENERAL NOTE HISTORY/);h.node('#tcu-pause').onclick();assert.equal(h.node('#tcu-pause-dialog').open,true);
 h.node('#tcu-confirm-pause').onclick();assert.match(h.node('#tcu-pause-error').textContent,/required/);assert.equal(h.calls.length,0);
 h.node('#tcu-pause-note').value='End of shift';h.node('#tcu-confirm-pause').onclick();await flush();assert.equal(h.calls[0].path,'/api/tineco-toc/pause');assert.equal(h.calls[0].body.pauseNote,'End of shift');assert.equal(h.calls[0].body.data.partUsedDetail,'Motor\nFilter');assert.equal(h.saved.phase,'paused');assert.match(h.node('#tcu-app').innerHTML,/UNIT PAUSED/);assert.match(h.node('#tcu-app').innerHTML,new RegExp(['Pre-QC','Repair','Final QC'][step]));h.module.cleanup();
});
test('failed pause keeps the exact pending request for refresh recovery',async()=>{
 const h=setup({phase:'draft',requestId:'visit-1234567890123456',recordId:'existing',unitId:'TCU-old',sn:'SN',entry:2,step:1,clientStatus:'Pending',previousLaborMinutes:15,startedAt:Date.now(),data:{partUsedDetail:'Motor',totalPartsUsed:'1',repairLevel:'Level 1'}});h.window.MkiteApiClient.post=async()=>({ok:false,error:{code:'CONNECTION_ERROR',message:'Retry'}});
 h.t.run('pause',{requestId:h.saved.requestId,pauseNote:'Break',data:h.saved.data});await flush();const pending=structuredClone(h.saved.pending);assert.equal(pending.action,'pause');assert.match(h.node('#tcu-app').innerHTML,/RETRY SAVE/);h.module.cleanup();h.module.init(h.ctx);await flush();assert.deepEqual(h.saved.pending,pending);h.module.cleanup();
});
