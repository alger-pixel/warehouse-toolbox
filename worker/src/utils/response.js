...
export function corsHeaders(request, env) {
  ...
  if (origin && allowedOrigins(env).has(origin)) { headers["Access-Control-Allow-Origin"] = origin; headers["Access-Control-Allow-Methods"] = "GET, POST, PUT, PATCH, DELETE, OPTIONS"; headers["Access-Control-Allow-Headers"] = "Content-Type, X-Request-ID, Authorization"; headers["Access-Control-Max-Age"] = "86400"; }
  return headers;
}
